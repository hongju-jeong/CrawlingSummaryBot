...
- 일일 다이제스트 설명에는 LangChain 기반 RAG를 적용해 관련 기사 맥락을 함께 반영합니다.
- 프론트 대시보드에서 진행 상황, 이슈 리스트, AI 요약, 전송 로그를 실시간으로 확인할 수 있습니다.
- 프론트 왼쪽 탭에서 `모니터링`과 `전날 요약` 화면을 분리해 확인할 수 있습니다.
- `전날 요약` 탭에서 요약 테스트 버튼으로 daily digest를 바로 수동 실행할 수 있습니다.
...
- 일일 키워드 다이제스트
  ...
  - LangChain + OpenAI Embeddings 기반 RAG 설명 생성
- 실시간 모니터링 UI
  - 크롤링 프로세스 수
  - 기사별 처리 단계
  - 실시간 이벤트 피드
...
- 수동 실행/자동 실행 제어
  ...
  - 수동 수집 중단 지원
  - Swagger와 프론트에서 daily digest 수동 실행 가능
...
- AI: OpenAI Responses API, LangChain, LangChain OpenAI
...
8. daily digest는 사건 클러스터 기반 키워드 선정 후 RAG로 설명 생성
9. 프론트 대시보드와 전날 요약 탭에 반영
